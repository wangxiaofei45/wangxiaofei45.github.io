...
$(function() {
    ...
    $("#show_style").click(function() {
        if ($("#fxproduct_list").hasClass("grid")) {
            $(this).find("span").removeClass("browse-grid").addClass("browse-list");
            $("#fxproduct_list").removeClass("grid").addClass("list")
        } else {
            $(this).find("span").addClass("browse-grid").removeClass("browse-list");
            $("#fxproduct_list").addClass("grid").removeClass("list")
        }
    });
    ...
    $("#fxproduct_list").on("click", ".goods-store a", function() {
        var e = $(this);
...
function get_list() {
    ...
    $.getJSON(ApiUrl + "/index.php?act=goods&op=fxgoods_list" + window.location.search.replace("?", "&"), param, function(result) {
        if (!result) {
            result = [];
            result.datas = [];
        ...
        $("#fxproduct_list .goods-secrch-list").append(html);
...
function search_adv() {
    $.getJSON(ApiUrl + "/index.php?act=index&op=search_adv", function(result) {
        ...
        $("#search_submit").click(function() {
            ...
            window.location.href = WapSiteUrl + "/tmpl/fxproduct_list.html" + s_str
        });
...
function init_get_list(e, r) {
    ...
    curpage = 1;
    hasmore = true;
    $("#fxproduct_list .goods-secrch-list").html("");
